...
public class DoorGuardCrcUtil {

    public static byte[] getCRCByteData(byte[] bt) {
        int sum = 0;
        for (byte i = 1; i < bt.length - 2; i++) {
            if (bt[i] < 0) {
                int temp = Integer.parseInt(BLEByteUtil.bytesToHexString(new byte[]{bt[i]}).trim(), 16);
                sum += temp;
                continue;
            }
            sum += bt[i];
        ...
        crcbyte[0] = (byte) (sum >> 8);
        crcbyte[1] = (byte) sum;
        return crcbyte;
    }

    public static byte[] getCRCByteValue(byte[] bt) {
        byte[] tempbyte = new byte[bt.length - 2];
        System.arraycopy(bt, 0, tempbyte, 0, tempbyte.length);
        char ch = caluCRC(tempbyte);
